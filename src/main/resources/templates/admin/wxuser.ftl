...
<table class="easyui-datagrid" id="query"  title="DataGrid Complex Toolbar"
       data-options="rownumbers:true,singleSelect:true,url:'http://localhost:8080/wxuser/queryall',method:'get',toolbar:'#tb',footer:'#ft',
       pagination:true,pageNumber:1,pageList:[5,10,15,20,25],fitColumns:true">
    <thead>
    <tr>
        <th data-options="field:'wechatAuthId',width:80">微信授权id</th>
        <th data-options="field:'userId',width:100">用户编号</th>
        <th data-options="field:'openid',width:100">用户openid</th>
...
<div id="dlg" class="easyui-dialog" style="width:400px;height:280px;padding:10px 20px"
    ...
    <form id="fm" method="post" novalidate>
        <div class="fitem">
            <label>wechatAuthId</label>
            <input id="wechatAuthId" name="wechatAuthId" class="easyui-textbox" required="true">
        </div>
        <div class="fitem">
            <label>userId</label>
...
<script language="JavaScript" type="text/javascript">
    ...
    function destroyUser(){
        ...
        if (row){
            $.messager.confirm('Confirm','Are you sure you want to destroy this user?',function(r){
                if (r){
                    $.post('http://localhost:8080/wxuser/delete',{wechatAuthId:row.wechatAuthId},function(result){
                        if (result=='1'){
                            alert("删除成功");
                            $('#query').datagrid('reload');	// reload the user data
                        } else {
    ...
    function editUser(){
        ...
        if (row){
            ...
            $('#fm').form('load',row);
            url = 'http://localhost:8080/wxuser/update?Id='+row.wechatAuthId;
        }
    }
</script>
